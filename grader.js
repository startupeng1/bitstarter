...
var processHtmlBuffer = function(htmlBuffer, checks) {
...
if(require.main == module) {
	...
	processHtmlBuffer(fs.readFileSync(program.file), program.checks);
    }
    else if (program.url != null) {
        rest.get(program.url).on('complete', function(result, response) {
            if (result instanceof Error) {
                console.error('Error fetching url %s %s', program.url,  util.format(response.message));
            } else {
                processHtmlBuffer(result, program.checks);
            }
        });
    }
} else {
